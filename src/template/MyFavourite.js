...
import DialogDisplayMyFav from './DialogDisplayMyFav';
...
function FormRow(data) {
  return (
    <React.Fragment>
      {
        data.exchangeData.map((x, k) => (
          <Grid item xs={3} key={k}>
            <Item>
              <img src={x.image} alt={x.name} height="120" width="120" />
              <p> {x.name} </p>
              <p> {x.current_price} </p>
              <p> {x.last_updated} </p>
            </Item>
    ...
    fetch(url).then((response) => {
      response.json().then((datas) => {
        ...
        const results = datas.filter((item) => {
          const is_listed = fav.some(itemz => item.id === itemz);
          return is_listed
        });
      ...
      field: 'name', headerName: 'Name', minWidth: '200', renderCell: (params) => {
        ...
        return (
          <div>
            ...
            <span>  {params.row.name} <Link to={`/ExchangeDetail/${params.row.id}`}>More Info</Link></span>
           </div>
        )
      }
    },
